...
' . trim($content->html) .
	...
	private static function generateUrlContents(Resource $resource, Content $content)
	{
        ...
        if ('' !== trim($content->url)) {
            $html .= '<div class="col-xs-? col-sm-? col-md-? col-lg-? pull-right" style="text-align: center;width:50%;"><img
class="content-img"
src="' . $content->url . '"
width="250px" onclick="imageClicked(this)"></div>';
        }
        if ('' !== trim($content->html)) {
            $html .= '<div class="col-xs-? col-sm-? col-md-? col-lg-? pull-left" style="text-align: left;margin-top:
             8px;width:50%;">
'
				. $content->html
				. '</div>';
